...
//Output:
//	None	
void initField(int num_of_mine) {               // initialize the field with unknown cells
	//Complete me
    for (int i = 0; i < height; i++)            // loop through the rows
    {      
        for (int j = 0; j < width; j++)         // loop through the columns
        {
            setCell(i, j, UNKNOWN);             // set the cell to be unknown
        }
    }

    setMine(num_of_mine);                       // set the mines
...
bool isMine(int i, int j) {                     // check if the cell is a mine                                    
	//Complete me
    if(i < 0 || i >= height || j < 0 || j >= width) // check if the cell is out of the field
	{
		return false;
	}
	else if(getCell(i, j) == UNFLAGGED_MINE || getCell(i, j) == FLAGGED_MINE) // check if the cell is a mine
	{
		return true;		// return true if the cell is a mine
	}
...
//Output
//	None	 
void reveal(int i,int j) {                      // reveal the cell
	//Complete me
    // if the cell is a mine and not flagged, then the player loses the game
    if(getCell(i , j) == UNFLAGGED_MINE)
    {
        explode = true;		// set the explode variable to true
    }

    int temp = 0;

    // check the surrounding cells for mines using for loops and the isMine function
    for(int x = -1; x <= 1; x++)			// loop through the rows
    {
        for(int y = -1; y <= 1; y++)		// loop through the columns
        {
            if(!(y == 0 && x == 0) && isMine(i + y, j + x))		// check if the cell is a mine 
            {
    ...
    setCell(i, j, temp);                        // set the cell to the number of mines in the surrounding cells


	// if the cell is not a mine and there is no mine in the surrounding cells, then reveal all surrounding cells using recursion
	// here are some of the resources that i used to implement the recursion function https://www.programiz.com/cpp-programming/recursion, https://www.w3schools.com/cpp/cpp_functions_recursion.asp, https://www.geeksforgeeks.org/recursive-functions/
	...
	if(temp == 0)								
	{
		for(int x = -1; x <= 1; x++)			// loop through the rows
		{
			for(int y = -1; y <= 1; y++)		// loop through the columns
			{
				if(!(y == 0 && x == 0) && getCell(i + y, j + x) == UNKNOWN)		// check if the cell is unknown
				{
					reveal(i + y, j + x);		// call the reveal function
				}
			}
		}
	}
...
//The flag function flags a cell as a mine
//Input:
//	i: the row index
//	j: the column index
//Output:
...
void flag(int i, int j)                         // flag the cell
{
    //Complete me

    if(getCell(i , j) == UNKNOWN)				// check if the cell is unknown
    {
        num_incorrect_flagged_mine++;			// increment the number of incorrect flagged mines
        setCell(i, j, INCORRECT_FLAGGED_MINE);	// set the cell to be incorrect flagged mine
    }
    else if(getCell(i, j) == UNFLAGGED_MINE)	// check if the cell is unflagges mine
    {
        num_correct_flagged_mine++;				// increment the number of correct flagged mines
        setCell(i, j, FLAGGED_MINE);			// set the cell to be flagged mine
    }
    else if(getCell(i, j) == INCORRECT_FLAGGED_MINE)	// check if the mine is incorrect flagged mine
    {
        num_incorrect_flagged_mine--;					// decrement the number of incorrect flagged mines
        setCell(i, j, UNKNOWN);					// set the cell to be unknown
    }
}

void unflag(int i, int j)						// unflag the cell
{

    if(getCell(i, j) == FLAGGED_MINE)			// check if the cell is flagged mine
    {
        setCell(i, j, UNFLAGGED_MINE);			// set the cell to be unflagged mine
        num_correct_flagged_mine--;				// decrement the number of correct flagged mines
    }
    else if(getCell(i, j) == INCORRECT_FLAGGED_MINE)	// check if the cell is incorrect flagged mine
    {
        setCell(i, j, UNKNOWN);					// set the cell to be unknown
        num_incorrect_flagged_mine--;			// decrement the number of incorrect flagged mines
    }
...
//Output:
//	None  
void executeCmd(string cmd) {                   // execute the command
	//Comeplete me

    // get the command type
    if(cmd[0] == 'f')
    {
        int i = cmd[1] - '0';                  // get the row index
        int j = cmd[2] - '0';                  // get the column index

        flag(i, j);                            // flag the cell
    }
    else if(cmd[0] == 'r')
    {
        int i = cmd[1] - '0';                  // get the row index
